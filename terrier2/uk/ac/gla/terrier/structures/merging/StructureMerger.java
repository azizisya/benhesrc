...
public class StructureMerger {
	 ...
	 * Merges the two direct files and the corresponding document id files.
	 */
	protected void mergeDirectFilesWithDocnosInOrder() {
		try {
			...
			DirectIndexBuilder dfOutput = new DirectIndexBuilder(dfoutputName);
			...
			DirectIndex df1 = new DirectIndex(new DocumentIndex(docidFilename1));
			...
			String dfInputName2 = invertedFile2.substring(0,invertedFile2.lastIndexOf(".")) + 
											  ApplicationSetup.DF_SUFFIX;
			BitInputStream dfInput2 = new BitInputStream(dfInputName2);
			DirectIndex df2 = new DirectIndex(new DocumentIndex(docidFilename2));
			...
			int read1 = docidInput1.readNextEntry();
			int read2 = docidInput2.readNextEntry();
			while (read1!=-1||read2!=-1){
				String docno1 = docidInput1.getDocumentNumber();
				String docno2 = docidInput2.getDocumentNumber();
				int compare = docno1.compareTo(docno2);
				if (compare>0){
					if (read2!=-1){
						// write
						if (docidInput2.getDocumentLength()>0){
							int[][] terms = df2.getTerms(docidInput2.getDocumentId());
							...
							FilePosition post = dfOutput.addDocument(terms);
							docidOutput.addEntryToBuffer(docidInput2.getDocumentNumber(), 
									docidInput2.getDocumentLength(), post);
						}
						read2 = docidInput2.readNextEntry();
					}
				}else if (compare < 0){
					if (read1!=-1){
						if (docidInput1.getDocumentLength()>0){
							int[][] terms = df1.getTerms(docidInput1.getDocumentId());
							FilePosition post = dfOutput.addDocument(terms);
							docidOutput.addEntryToBuffer(docidInput1.getDocumentNumber(),
									docidInput1.getDocumentLength(), post);
						}
						read1 = docidInput1.readNextEntry();
					}
				}else{
					if (docidInput2.getDocumentLength()>0){
						int[][] terms = df2.getTerms(docidInput2.getDocumentId());
	...
	/** 
	 * creates the final term code to offset file.
	 */
	protected void createLexidFile(String indexPath, String indexPrefix, int maxTermid) {
		try {
		
			String lexOutputName = indexPath+ApplicationSetup.FILE_SEPARATOR+indexPrefix+ApplicationSetup.LEXICONSUFFIX;

			int numOfTerms=0;
			if (ApplicationSetup.getProperty("string.use_utf","false").equals("true")) {
				java.io.File lex = new java.io.File(lexOutputName);
			...
			LexiconInputStream lexInput = null;
			if (ApplicationSetup.getProperty("string.use_utf","false").equals("true")) {
				lexInput = new UTFLexiconInputStream(lexOutputName);
				int termCounter = 0;
				while (lexInput.readNextEntry()>=0) {
					offsets[lexInput.getTermId()] = termCounter * UTFLexicon.lexiconEntryLength;
					termCounter++; 
				}
				lexInput.close();
			} else {
				lexInput = new LexiconInputStream(lexOutputName);
				int termCounter = 0;
				while (lexInput.readNextEntry()>=0) {
					offsets[lexInput.getTermId()] = termCounter * Lexicon.lexiconEntryLength;
					termCounter++; 
				}
				lexInput.close();
			}
						
			String lexidOutputName = indexPath+ApplicationSetup.FILE_SEPARATOR+indexPrefix+".lexid";
			DataOutputStream dos = new DataOutputStream(new BufferedOutputStream(new FileOutputStream(lexidOutputName)));
	 ...
	 * creates the final term code to offset file.
	 */
	protected void createLexidFile(String indexPath, String indexPrefix) {
		try {
			...
			long[] offsets = new long[numOfTerms];
			LexiconInputStream lexInput = null;
			if (ApplicationSetup.getProperty("string.use_utf","false").equals("true")) {
				lexInput = new UTFLexiconInputStream(lexOutputName);
				int termCounter = 0;
