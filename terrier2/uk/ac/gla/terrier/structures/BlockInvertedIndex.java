...
public class BlockInvertedIndex extends InvertedIndex implements IndexConfigurable {
	...
	public int[][] getDocuments(final long startOffset, final byte startBitOffset, final long endOffset, final byte endBitOffset, final int df, final boolean[] flag){
		final int fieldCount = FieldScore.FIELDS_COUNT;
		final boolean loadTagInformation = FieldScore.USE_FIELD_INFORMATION;
		
		final int[][] documentTerms = new int[5][];
		documentTerms[0] = flag[0]? new int[df]:null;
		documentTerms[1] = flag[1]? new int[df]:null;
		documentTerms[2] = flag[2]? new int[df]:null;
		documentTerms[3] = flag[3]? new int[df]:null;
		final TIntArrayList blockids = new TIntArrayList(df); //ideally we'd have TF here
		...
		try{
			...
			if (loadTagInformation) { //if there are tag information to process
				//documentTerms[2] = new int[df]; 
				docid = file.readGamma() - 1;
				if (flag[0])
					documentTerms[0][0] = docid;
				int tf = file.readUnary();
				if (flag[1])
					documentTerms[1][0] = tf;
				int fieldScore = file.readBinary(fieldCount);
				if (flag[2])
					documentTerms[2][0] = fieldScore;
				int blockfreq = file.readUnary() - DocumentBlockCountDelta;
				if (flag[3])
					documentTerms[3][0] = blockfreq;
				int tmpBlocks[] = (flag[4])?new int[blockfreq]:null;
				int previousBlockId = -1;
				for(int j=0;j<blockfreq;j++)
				{
					 previousBlockId = file.readGamma() + previousBlockId;
					 if (flag[4])
						 tmpBlocks[j] = previousBlockId;
				}
				if (flag[4])
					blockids.add(tmpBlocks);
				
				for (int i = 1; i < df; i++) {					
					docid  = file.readGamma() + docid;
					if (flag[0])
						documentTerms[0][i] = docid;
					tf = file.readUnary();
					if (flag[1])
						documentTerms[1][i]  = tf;
					fieldScore = file.readBinary(fieldCount);
					if (flag[2])
						documentTerms[2][i]  = fieldScore;
					blockfreq = file.readUnary() - DocumentBlockCountDelta;
					if (flag[3])
						documentTerms[3][i] = blockfreq;
					tmpBlocks = (flag[4])?new int[blockfreq]:null;
					previousBlockId = -1;
					for(int j=0;j<blockfreq;j++)
					{
						previousBlockId = file.readGamma() + previousBlockId;
	...
	public int[][] getDocumentsForModifiers(final long startOffset, final byte startBitOffset, final long endOffset, final byte endBitOffset, final int df){
		final int fieldCount = FieldScore.FIELDS_COUNT;
		final boolean loadTagInformation = FieldScore.USE_FIELD_INFORMATION;
		
		final int[][] documentTerms = new int[5][];
		documentTerms[0] = new int[df]; // docids
		documentTerms[1] = null; // ignore frequencies
		int frequency = 0;
		int fieldScore = 0;
		documentTerms[2] = null; // ignore field scores
		documentTerms[3] = new int[df];
		final TIntArrayList blockids = new TIntArrayList(df); //ideally we'd have TF here
