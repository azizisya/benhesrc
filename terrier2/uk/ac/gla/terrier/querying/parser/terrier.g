...
query returns [Query q]
  {q=null;}
  : { /*System.out.print("implied ");*/ } q=impliedMultiTermQuery
  | { /*System.out.print("explicit ");*/ } q=explicitMultiTermQuery
  | q=phraseQuery
  | q=fieldQuery
  | q=requirementQuery
  | q=singleTermQuery
...
singleTermQuery returns [Query q]
  ...
  : qt:ALPHANUMERIC { stq.setTerm(qt.getText()); }
   (HAT {selector.push("numbers");}
		...
		|w_nf:NEG_NUM_FLOAT {stq.setWeight(Double.parseDouble(w_nf.getText())); selector.pop();}
		|w_i:NUM_INT {stq.setWeight(Double.parseDouble(w_i.getText())); selector.pop();}
		|w_ni:NEG_NUM_INT {stq.setWeight(Double.parseDouble(w_ni.getText())); selector.pop();}
		)
	)?
   {q = stq;}
  ;
...
//functionQuery returns [Query q]
//   { q = null;
//     FunctionQuery fq = new FunctionQuery();
//     Query child = null; }
//     
//   : fn:ALPHANUMERIC { fq.setFName(fn.getText()); } OPEN_SQUARE_PAREN
//     ( child = singleTermQuery { fq.add(child); } )+
//     ( cPar:CLOSE_SQUARE_PAREN
//       ( HAT { selector.push("numbers"); }
//         ( fWeight:NUM_FLOAT { fq.setWeight(Double.parseDouble(fWeight.getText())); selector.pop(); }
//           | iWeight:NUM_INT { fq.setWeight(Double.parseDouble(iWeight.getText())); selector.pop(); }
//         )
//       )?
//     )
//
...
functionQuery returns [Query q]
  ...
  : fn:ALPHANUMERIC { fq.setFName(fn.getText()); } OPEN_SQUARE_PAREN
    ( { /*System.out.print("argument ");*/ } child=argumentMultiTermQuery { fq.add(child); } )+
    ( cPar:CLOSE_SQUARE_PAREN
      ( HAT { selector.push("numbers"); }
        ( fWeight:NUM_FLOAT { fq.setWeight(Double.parseDouble(fWeight.getText())); selector.pop(); }
          | iWeight:NUM_INT { fq.setWeight(Double.parseDouble(iWeight.getText())); selector.pop(); }
...
argumentMultiTermQuery returns [Query q]
  { q= null; MultiTermQuery mtq = new MultiTermQuery(); Query child = null;}
  : OPEN_PAREN
    ( child=singleTermQuery { mtq.add(child); } )+
    ( closeP:CLOSE_PAREN
      ( HAT { selector.push("numbers"); }
        ( fWeight:NUM_FLOAT { mtq.setWeight(Double.parseDouble(fWeight.getText())); selector.pop(); }
          | iWeight:NUM_INT { mtq.setWeight(Double.parseDouble(iWeight.getText())); selector.pop(); }
        )
      )?
    )
...
impliedMultiTermQuery returns [Query q]
  ...
  : (( child=functionQuery
       ...
       | child=phraseQuery
       | { /*System.out.print("explicit ");*/ } child=explicitMultiTermQuery
