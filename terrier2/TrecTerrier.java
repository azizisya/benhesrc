...
public class TrecTerrier {
	...
	protected int processOptions(String[] args) {
		...
		if (!languagemodel && indexing && (direct||(!direct && !inverted)) &&  Files.exists(ApplicationSetup.DIRECT_FILENAME))
			return ERROR_DIRECT_FILE_EXISTS;
		
		if (!languagemodel && indexing && Files.exists(ApplicationSetup.INVERTED_FILENAME))
			return ERROR_INVERTED_FILE_EXISTS;
		
		if (inverted && !Files.exists(ApplicationSetup.DIRECT_FILENAME)) 
