 ...
 *
 */
public class DFEQFeature extends PSIFeature {
	...
	public DFEQFeature(Index index){
		super(index);
		...
		if (CACHED){
			...
			*/
			...
			for (int i=0; i<expTermids.length; i++){
				...
				LexiconEntry lexEntry = (LexiconEntry)lexicon.getLexiconEntry(expTermids[i]).getValue();
				// if (lexEntry.TF>collStats.getNumberOfDocuments())
				if ((double)lexEntry.getDocumentFrequency() > 0.05 * collStats.getNumberOfDocuments())
					cooccurrences += 0;
				else
					cooccurrences += IndexUtility.getCooccurrenceInCollection(expTermids[i], toRetain, index.getInvertedIndex(), index.getLexicon(), collStats.getNumberOfDocuments());
			}
			featureMap.put(featureId, (double)cooccurrences/expTermids.length);
		}
	}
		...
		try{
			...
			while ((line=br.readLine())!=null){
				...
				if (!cache.containsKey(queryid)){
					...
					map.put(docid, new Double(value));
					cache.put(queryid, map);
				}else{
					cache.get(queryid).put(docid, new Double(value));
					// System.out.println(queryid+"."+docid+", "+value);
				}					
			}
			br.close();
		}catch(IOException e){
	...
	public static void main(String[] args){
		...
		if (args[0].equals("--preprocess")){
			...
			DFEQFeature app = new DFEQFeature(Index.createIndex(indexpath, indexprefix));
		...
		}else if (args[0].equals("--preprocesspart")){
			String indexpath = args[1];
			String indexprefix = args[2];
			String qrelsFilename = args[3];
			// String outputFilename = args[4];
