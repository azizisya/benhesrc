...
public class SystemUtility {
    ...
    public static Query getRandomQuery(Lexicon lexicon, CollectionStatistics collSta){
...
//		System.out.println("queryType = " + queryType);
		...
		TermTree termTree = getRandomTermTree(lexicon, collSta, numberOfTerms, g);
		if (termTree.getNumberOfNodes() == 0)
			return null;
			
		String queryNumber = new String("" + queryType) + new String("" + System.currentTimeMillis());
			
		return new Query(termTree, queryNumber);	
	}
    
    public static TermTree getRandomTermTree(Lexicon lexicon, CollectionStatistics collSta, int numberOfTerms, int g){
    		
    	TermTree termTree = new TermTree();
		
		int avgTF = (int)(collSta.getNumberOfTokens() 
					/ collSta.getNumberOfUniqueTerms());
			
		for (int i = 0; i < numberOfTerms; i++){
			int termCode = (int)(Math.random() * collSta.getNumberOfUniqueTerms());
			lexicon.seekEntry(termCode);
			if (g != -1 && lexicon.getTF() < g * avgTF){
				i--;
				continue;
			}
			String term = lexicon.getTerm();
			termTree.insert(term);
		}
	...
	public static QueryExpansionModel initQueryExpansionModel(String qemethodName, 
		...
		qemodel.setCollectionLength(numOfTokens);
		return qemodel;
	}
