		...
		if (CACHED){
			...
			*/
			...
			for (int i=0; i<expTermids.length; i++){
				...
				LexiconEntry lexEntry = lexicon.getLexiconEntry(expTermids[i]);
				// if (lexEntry.TF>collStats.getNumberOfDocuments())
				if ((double)lexEntry.n_t > 0.05 * collStats.getNumberOfDocuments())
					cooccurrences += 0;
				else
					cooccurrences += IndexUtility.getCooccurrenceInCollection(expTermids[i], toRetain, index.getInvertedIndex(), collStats.getNumberOfDocuments());
			}
