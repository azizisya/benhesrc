...
public class Inverted2DirectIndexBuilder {
		...
		throws IOException
	{
		...
		{
			int docLength = docidStream.getDocumentLength();
			if (docLength > 0)
				tokens += docidStream.getDocumentLength();
			else if (docLength < 0	)
				logger.warn("Document "+docidStream.getDocumentId()+" with negative document length: "+docidStream.getDocumentLength());
	...
	protected int scanDocumentIndexForTokens(
			...
			throws IOException
		{
			...
			while (currentDocid < N){
				if (currentDocid == 1201509){
					currentDocid = 1246356;
					return currentDocid;
				}
				int docLength = docidx.getDocumentLength(currentDocid++);
				if (docLength > 0)
					tokens += docLength;
				else if (docLength < 0	)
					logger.warn("Document "+(currentDocid-1)+" with negative document length: "+docLength);
				if (tokens > processTokens)
					break;
			}
			return currentDocid;
