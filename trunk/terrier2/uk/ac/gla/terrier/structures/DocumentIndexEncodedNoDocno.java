 ...
 */
public class DocumentIndexEncodedNoDocno extends DocumentIndexEncoded {
	/** The logger used */
	private static Logger logger = Logger.getRootLogger();
	/** The entry length minus 4 bytes for the docid.*/
	private int shortEntryLength = this.entryLength-4-this.DOCNO_BYTE_LENGTH;
	...
	public DocumentIndexEncodedNoDocno(String path, String prefix)
	{
		this(path+ ApplicationSetup.FILE_SEPARATOR + prefix + ApplicationSetup.DOC_INDEX_SUFFIX);
	}
	public DocumentIndexEncodedNoDocno() {
	 ...
	 */
	public DocumentIndexEncodedNoDocno(String filename) {
		super(filename);
		try {
			DataInputStream dis = new DataInputStream(Files.openFileStream(filename));
			numberOfDocumentIndexEntries =
	...
	public String getDocumentNumber(int docid) {
		logger.warn("DOCNOS NOT SUPPORTED");
		return null;
	}
	/**
	...
	protected byte getEndBitOffset() {
		return bytes[index*shortEntryLength+12];
	}
	/**
	...
	protected long getEndOffset() {
		int i=index * shortEntryLength+4;
	...
	public void loadIntoMemory(DataInputStream dis, int numOfEntries)
		...
		// shortEntryLength = this.entryLength-4-this.DOCNO_BYTE_LENGTH;// 4 for skipping doc id
		...
		*/
		for (int i = 0; i < numOfBytes; i+=shortEntryLength) {
			...
			// load doc length
			dis.readFully(bytes, i, 4);
			// skip docno
			dis.skipBytes(DOCNO_BYTE_LENGTH);
			// load the rest
			dis.readFully(bytes, i+4, shortEntryLength-4);
			//n++;
		}
	...
	public void printBytes(int docid){
		...
		for (int i=0; i<shortEntryLength-4;i++)
			System.out.print(bytes[idx+i]);
		System.out.println();
	}
	...
	/**
	 * A main method for testing the DocumentIndexEncodedNoDocno class.
	 * <br>
	 ...
	 * <tt>java -cp ... uk.ac.gla.terrier.structures.DocumentIndexEncodedNoDocno filename --docno 1023</tt><br>
	...
	public static void main(java.lang.String[] args) {
		DocumentIndexEncodedNoDocno docIndexEncoded = new DocumentIndexEncodedNoDocno(args[0]);
