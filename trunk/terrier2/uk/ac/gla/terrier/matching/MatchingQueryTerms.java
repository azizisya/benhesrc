...
public class MatchingQueryTerms implements Serializable,Cloneable{
	...
	protected String[] termsRankedByWeights;
	...
	public void normaliseTermWeights(){
		int[] termWeightRanking = new int[termProperties.size()];
		double[] termWeights = new double[termProperties.size()];
		short[] dummy = new short[termProperties.size()];
		String[] termStrings = this.getTerms();
		termsRankedByWeights = new String[termProperties.size()];
		...
		for (int i = 0; i < termProperties.size(); i++){
			QueryTermProperties term = (QueryTermProperties)termProperties.get(termStrings[i]);
			termWeightRanking[i] = i;
			maxWeight = Math.max(maxWeight,term.weight);
			termWeights[i] = term.weight;
		}
		// normalise
		for (int i = 0; i < termProperties.size(); i++)
			((QueryTermProperties)termProperties.get(termStrings[i])).weight /= maxWeight;
		HeapSort.descendingHeapSort(termWeights, termWeightRanking, dummy, termProperties.size());
		
		for (int i=0; i<termStrings.length; i++){
			termsRankedByWeights[i] = termStrings[termWeightRanking[i]]; 
		}
	}
	 
	public String[] getTermsWithHighestWeights(int topx){
		int numberOfReturnedTerms = Math.min(topx, termProperties.size());
		String[] terms = new String[numberOfReturnedTerms];
		for (int i=0; i<numberOfReturnedTerms; i++){
			terms[i] = termsRankedByWeights[i];
		}
		return terms;
	}
	...
	public boolean hasTerm(String term){
		return termProperties.keySet().contains(term);
	}
