 ...
 * <b>Properties</b>
 * <ul>
 * <li><tt>dependency.type</tt> - one of SD, FD for sequential dependence or full dependence</li>
 ...
 * <li><tt>w_t</tt> - weight of unigram in combination</li>
 * <li><tt>w_o</tt> - weight of SD in combination</li>
 * <li><tt>w_u</tt> - weight of FD in combination</li>
 * </ul>
 * @author Vassilis Plachouras
 * @version $Revision: 1.1 $
 */
public class ProximityScoreModifierECIR2008 implements DocumentScoreModifier {
	public Object clone()
	{
		return new ProximityScoreModifierECIR2008(phraseTerms);
	}
	...
	protected String dependency = ApplicationSetup.getProperty("dependency.type","");

  
	protected double w_t = Double.parseDouble(ApplicationSetup.getProperty("w_t","1.0d"));
	protected	double w_o = Double.parseDouble(ApplicationSetup.getProperty("w_o","1.0d"));
	protected	double w_u = Double.parseDouble(ApplicationSetup.getProperty("w_u","1.0d"));
	...
	public ProximityScoreModifierECIR2008() { } 	
	public ProximityScoreModifierECIR2008(final String[] pTerms) {
		phraseTerms = pTerms;
	}

	public ProximityScoreModifierECIR2008(final String[] pTerms, boolean r) {
		this(pTerms);
		required = r;
	}
	...
	public String getName() {
		return "ProximityScoreModifierECIR2008";
	}
...
//////// end score
		...
		final double avgDocLen =  ((double)(numTokens - numDocs *(ngramLength-1))) / (double)numDocs;
		final double avgDocLen_SD = ((double)(numTokens - numDocs*(ngramLength_SD - 1))) / (double)numDocs;
		...
		if (dependency.equals("FD")){
		  for (int i=0; i<phraseLength-1; i++) {
		  	for (int j=i+1; j<phraseLength; j++) {
		  		...
		  		termid1 = index.getLexicon().getLexiconEntry(term1).getTermId();
		  		termid2 = index.getLexicon().getLexiconEntry(term2).getTermId();
  				...
  				for (int k=0; k<docidsLength; k++) {
					...
					int docLength = 0;
					try{
						doi.getDocumentLength(docids[k]);	
					}catch(Exception ioe){
						ioe.printStackTrace();
					}
	...
	protected int[][] getPostings(int termid, Index index) {
		...
		if (postingsCache.contains(termid)) {
			...
			postings = index.getInvertedIndex().getDocuments((LexiconEntry)index.getLexicon().getLexiconEntry(termid).getValue());
