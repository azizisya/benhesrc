...
public class ArffFeatureExtractor implements DocumentFeatureExtractor{
	...
	public static TIntIntHashMap processQuery(String query, Index index){
		...
		BaseTermPipelineAccessor pipeline = BaseTermPipelineAccessor.getDefaultPipelineAccessor();
		Lexicon lexicon = index.getLexicon();
		for (int i=0; i<tokens.length; i++){
			String term = pipeline.pipelineTerm(tokens[i].toLowerCase());
			...
			if (term!=null){
				...
				if (lexEntry!=null){
					map.adjustOrPutValue(lexEntry.getTermId(), 1, 1);
				}
			}
		}
		return map;
