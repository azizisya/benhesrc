...
import uk.ac.gla.terrier.utility.TerrierTimer;
 ...
 public class DistributedThreeManager extends ThreeManager
 {
	...
	private static Logger logger = Logger.getLogger("broker");
 	...
 	/**
 	 * The second-pass retrieval parameter setting on body index. 
 	 */
 	public double cBodyPost = Double.parseDouble(
 			ApplicationSetup.getProperty("c.body.post", 
 					ApplicationSetup.getProperty("c.body", "6")));
 	/**
 	 * The second-pass retrieval parameter setting on anchor text index. 
 	 */
 	public double cAtextPost = Double.parseDouble(
 			ApplicationSetup.getProperty("c.anchor.post", 
 					ApplicationSetup.getProperty("c.anchor", "10")));
 	/**
 	 * The second-pass retrieval parameter setting on title index. 
 	 */
 	public double cTitlePost = Double.parseDouble(
 			ApplicationSetup.getProperty("c.title.post", 
 					ApplicationSetup.getProperty("c.title", "32")));
	...
	public int[] numberOfTopRankedDocs;
 	 ...
 	 *
 	 */
 	public DistributedThreeManager(){
 	...
 	protected void loadPostParameters(){
 		cBodyPost = Double.parseDouble(
 	 			ApplicationSetup.getProperty("c.body.post", 
 	 					ApplicationSetup.getProperty("c.body", "6")));
 	 	cAtextPost = Double.parseDouble(
 	 			ApplicationSetup.getProperty("c.anchor.post", 
 	 					ApplicationSetup.getProperty("c.anchor", "10")));
 	 	cTitlePost = Double.parseDouble(
 	 			ApplicationSetup.getProperty("c.title.post", 
 	 					ApplicationSetup.getProperty("c.title", "32")));
 	}
...
// 			terms.mergeExpansionTerms(threads[i].getTerms());		
	 ...
	 */
 	protected void updateStatistics() {
 		...
 		this.totalNumOfDocumentsInFields = new long[3];
 		this.totalNumOfTokensInFields = new long[3];
 		this.totalNumOfPointersInFields = new long[3];
	...
	protected void setNormalisation(String[] normalisationNames, double[] parameters,
		...
		double wqb = Double.parseDouble(ApplicationSetup.getProperty("body.qeweight", ""+weights[0]));
		double wqa = Double.parseDouble(ApplicationSetup.getProperty("anchor.qeweight", ""+weights[1]));
		double wqt = Double.parseDouble(ApplicationSetup.getProperty("title.qeweight", ""+weights[2]));
		double[] QEWeights = {wqb, wqa, wqt};
		for (int i = 0; i < this.numberOfServers; i++){
			try{
 	...
 	public void runPreProcessing(SearchRequest srq)
 	{
 		...
 		cValueBody = Double.parseDouble(ApplicationSetup.getProperty("c.body","1.0D"));
 		cValueAnchor = Double.parseDouble(ApplicationSetup.getProperty("c.anchor","1.0D"));
 		cValueTitle = Double.parseDouble(ApplicationSetup.getProperty("c.title","1.0D"));
 		
 		bodyNormalisationName = ApplicationSetup.getProperty("normalisation.body", "2");
 		atextNormalisationName = ApplicationSetup.getProperty("normalisation.atext", "2");
 		titleNormalisationName = ApplicationSetup.getProperty("normalisation.title", "2");
		...
		double[] weights = {this.BODY_WEIGHT, this.ANCHOR_WEIGHT, this.TITLE_WEIGHT};
 		DistributedMatchingQueryTerms queryTerms = 
 			new DistributedMatchingQueryTerms(rq.getQueryID(), 3);
 		...
 		if (cacheResults){
	 		...
	 		queryTerms.setParameterValues(parameters);
	 		queryTerms.setFieldWeights(weights);
	 		for (int i = 0; i < this.numberOfServers; i++){
	 			try{
 	...
 	public void setFirstPassRetrievalNormalisation(){
		...
		double[] parameters = {cValueBody, cValueAnchor, cValueTitle};
		double[] weights = {this.BODY_WEIGHT, this.ANCHOR_WEIGHT, this.TITLE_WEIGHT};
		this.setNormalisation(normalisationNames, parameters, weights);
 	}
 	/**
 	...
 	public void setSecondPassRetrievalNormalisation(){
		...
		double[] parameters = {cBodyPost, cAtextPost, cTitlePost};
		double[] weights = {this.BODY_WEIGHT, this.ANCHOR_WEIGHT, this.TITLE_WEIGHT};
		this.setNormalisation(normalisationNames, parameters, weights);
 	}
 	/**
