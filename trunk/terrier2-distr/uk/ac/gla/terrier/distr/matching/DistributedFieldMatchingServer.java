 ...
 * under the License.
 *
 * The Original Code is DistributedFieldMatchingServer.java.
 *
 ...
 */
public class DistributedFieldMatchingServer extends UnicastRemoteObject
	 ...
	 * The matching class.
	 */
	protected DistributedFieldMatching matching;
	 ...
	 */
	public DistributedFieldMatchingServer(
			String[] indexPath, String[] indexPrefix) throws RemoteException{
		int NumberOfFields = indexPath.length;
		Index[] indices = new Index[NumberOfFields];
		if (logger.isDebugEnabled())
			logger.debug("NumberOfFields: "+NumberOfFields);
		for (int i=0; i<NumberOfFields; i++)
			indices[i] = Index.createIndex(indexPath[i], indexPrefix[i]);	
		matching = new DistributedFieldMatching(indices);
...
//		setup log4j
		if (new File(ApplicationSetup.LOG4J_CONFIG).exists())
		{
			DOMConfigurator.configure(ApplicationSetup.LOG4J_CONFIG);
		}
		else
		{
			...
			new DOMConfigurator().doConfigure(new StringReader(ApplicationSetup.DEFAULT_LOG4J_CONFIG), org.apache.log4j.LogManager.getLoggerRepository());
		}
		if (logger.isDebugEnabled())
		...
		DistributedFieldMatchingServer match = null;
		String[] indexPath = new String[(args.length-1)/2];
		String[] indexPrefix = new String[(args.length-1)/2];
		int counter = 0;
		for (int i=1; i<args.length;i++){
			indexPath[counter] = args[i++];
			indexPrefix[counter++] = args[i]; 
		}
		try {
			
			match = new DistributedFieldMatchingServer(
					indexPath, indexPrefix);
			Naming.rebind(matchName, (DistributedMatchingServer)match);
			if (logger.isDebugEnabled())
				logger.debug("DistributedMatchingServer bound");
		} catch (Exception e) {
			e.printStackTrace();
			System.exit(1);
		}
