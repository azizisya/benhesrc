...
public class MatchingQueryTerms implements Serializable,Cloneable
{
	...
	static class StringQueryTermPropertiesByIndexComparator implements Comparator<Map.Entry<String,QueryTermProperties>>
	{
	...
	public void setTermProperty(String term) {
		...
		if (properties == null) {
			termProperties.put(term, new QueryTermProperties(termAdditionIndex++));
		}
	}
	...
	public String[] getTerms() {
		Map.Entry<String, QueryTermProperties>[] entries = termProperties.entrySet().toArray(new Map.Entry[termProperties.size()]);
		if (entries.length == 0)
			return tmpString;
		Arrays.sort(entries, BY_INDEX_COMPARATOR);
		final int l = entries.length;
		String[] terms = new String[l];
		for(int i=0;i<l;i++)
