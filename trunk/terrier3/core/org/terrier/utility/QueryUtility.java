...
public class QueryUtility {
	 ...
	 * @return
	 */
	public static int[] queryStringToTermids(String query, String[] pipes, Lexicon _lexicon){
		...
		BaseTermPipelineAccessor pipeline = new BaseTermPipelineAccessor(pipes);
		for (int i=0; i<tokens.length; i++){
			String term = tokens[i].trim().toLowerCase();
			term = pipeline.pipelineTerm(term);
			if (term!=null && term.trim().length() > 0){
				if (!termSet.contains(term)){
					try{
						...
						if (lexEntry!=null){
							termSet.add(term);
							termidSet.add(lexEntry.getTermId());
						}
	...
	public static int[] filterTerms(int[] termids, Lexicon lexicon, int numberOfDocuments, double ratio){
		...
		for (int i=0; i<termids.length; i++){
			LexiconEntry entry = (LexiconEntry)lexicon.getLexiconEntry(termids[i]).getValue(); 
			if (entry.getDocumentFrequency()<=upperDF)
