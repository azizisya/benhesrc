...
public class BlockDocumentSetEntropy{
	...
	public int[] filterQueryTerms(int[] termids){
		...
		for (int j=0; j<termids.length; j++){
			LexiconEntry lexEntry = lexicon.getLexiconEntry(termids[j]);
			if (lexEntry!=null){
				DF[j] = lexEntry.n_t;
			}else
				DF[j] = collSta.getNumberOfDocuments();
		}
	...
	public void printEntropyForResults(String resultFilename, String docIdxOutputFilename){
		...
		for (int i=0; i<queryids.length; i++){
			...
			double entropy = 0d;
			int[] termids = QueryUtility.queryStringToTermids(queries.getQuery(queryids[i]), manager, lexicon);
	...
	public static double getInfoPrior(LexiconEntry lexEntry, long numberOfTokens){
		return -Math.log((double)lexEntry.TF/numberOfTokens)/Math.log(2d);
	}
	...
	public void printMeanEntropy(String resultFilename, String docIdxOutputFilename, int topX){
		...
		for (int i=0; i<queryids.length; i++){
			...
			for (int termid : termids)
				System.err.print("("+lexicon.getLexiconEntry(termid).term+", "+termid+") ");
			System.err.println();
	...
	protected double getEntropies(
			Manager manager, String query, String[] docnos, TObjectIntHashMap<String> docmap, double alpha){
		double entropy = 0d;
		int[] termids = QueryUtility.queryStringToTermids(query, manager, lexicon);
		termids = this.filterQueryTerms(termids);
