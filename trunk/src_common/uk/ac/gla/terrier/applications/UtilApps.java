...
import uk.ac.gla.terrier.utility.ResultUtility;
import uk.ac.gla.terrier.utility.Rounding;
	...
	public class UtilApps {
		...
		public static void pipelineProcessOpinionTermList(String filename, String outputFilename){
			...
			try{
				...
				while ((str=br.readLine())!=null){
					...
					bw.write(manager.pipelineTerm(tokens[0])+" "+tokens[1]+ApplicationSetup.EOL);
				}
				br.close(); bw.close();
			}catch(IOException ioe){
				ioe.printStackTrace();
		...
		/**
		 * Remove any term in the wordlist that appear as the stemmed form of any stopword. 
		 * If a term appears too frequently in the collection, it will also be removed.
		 */
		public static void filterStemmedOpinionWordlist(String stopwordlist, String wordlist){
			...
			THashSet<String> stopwordSet = new THashSet<String>();
			try{
				BufferedReader br = Files.openFileReader(stopwordlist);
				String str = null;
				while ((str=br.readLine())!=null){
					String term = manager.pipelineTerm(str.trim());
					stopwordSet.add(term);
				}
				br.close();
				br = Files.openFileReader(wordlist);
				StringBuffer buf = new StringBuffer();
				while ((str=br.readLine())!=null){
					String[] tokens = str.split(" ");
					LexiconEntry lexEntry = lexicon.getLexiconEntry(tokens[0]);
					if (lexEntry!=null && !stopwordSet.contains(tokens[0]) && lexEntry.n_t<numberOfDocs/5){
							buf.append(str+ApplicationSetup.EOL);
					}
					else
						System.err.println("remove "+tokens[0]+" from opinion wordlist.");
				}
				br.close();
				BufferedWriter bw = (BufferedWriter)Files.writeFileWriter(wordlist+".filtered");
				bw.write(buf.toString());
				bw.close();
				index.close();
			}catch(IOException ioe){
		 ...
		 * If a term appears too frequently in the collection, it will also be removed.
		 */
		public static void filterOpinionWordlist(String wordlist){
			...
			int numberOfDocs = index.getCollectionStatistics().getNumberOfDocuments();
			try{
				BufferedReader br = Files.openFileReader(wordlist);
				StringBuffer buf = new StringBuffer();
				String str = null;
				while ((str=br.readLine())!=null){
					String[] tokens = str.split(" ");
					tokens[0] = manager.pipelineTerm(tokens[0]);
					if (tokens[0]!=null){
						LexiconEntry lexEntry = lexicon.getLexiconEntry(tokens[0]);
						if (lexEntry!=null && lexEntry.n_t<numberOfDocs/5){
							buf.append(str+ApplicationSetup.EOL);
							continue;
						}
					}
		...
		public static void main(String[] args) {
			if (args.length!=0){
				if (args[0].equals("--findnotin")){
				...
				}else if(args[0].equals("--processoptermlist")){
					// --processoptermlist filename outputfilename
					UtilApps.pipelineProcessOpinionTermList(args[1], args[2]);
				...
				}else if(args[0].equals("--convertqrels")){
					// --convertqrels docidxFilename feedbackFilename outputFilename
					IndexUtility.convertFeedbackQrels(args[1], args[2], args[3]);
					...
					// --filteropinionwordlist opinionwordlist
					UtilApps.filterOpinionWordlist(args[1]);
				}else if(args[0].equals("--filterstemmedopinionwordlist")){
					// --filterstemmedopinionwordlist stopwordlist opinionwordlist
					UtilApps.filterStemmedOpinionWordlist(args[1], args[2]);
				}else if (args[0].equals("--convertdocno2docidfromresults")){
					// --convertdocno2docidfromresults docIdxFilename, resultFilename, outputFilename
					IndexUtility.convertDocnoToDocidFromResults(args[1], args[2], args[3]);
					...
					// --printtermstats lexiconFilename termString
					IndexUtility.printTermStats(args[1], args[2]);
					...
					// --results2docnos docidxFilename resultFilename outputFilename
					IndexUtility.docids2docnosInResults(args[1], args[2], args[3]);
					...
					// --results2docids docidxFilename, resultFilename, outputFilename
					ResultUtility.convertDocnoToDocidFromResults(args[1], args[2], args[3]);
				}else if(args[0].equals("--qrels2docids")){
					// --qrels2docids docidxFilename feedbackFilename outputFilename
					IndexUtility.convertFeedbackQrels(args[1], args[2], args[3]);
				}else if (args[0].equals("--prels2docids")){
					// --prels2docids docidxFilename, feedbackFilename, outputFilename
					IndexUtility.prelsToDocids(args[1], args[2], args[3]);
				}else if (args[0].equals("--docnos2docids")){
					// --docnos2docids docidxFilename, docnosFilename, column, outputFilename
					IndexUtility.docnosToDocids(args[1], args[2], Integer.parseInt(args[3]), args[4]);
				}
