...
public class PartialMatching extends Matching {
	
	Lexicon globalLexicon = new Lexicon(ApplicationSetup.getProperty("global.lexicon", "must be given"));

	public PartialMatching() {
		// TODO Auto-generated constructor stub
	}

	public PartialMatching(Index index) {
		super(index);
		// TODO Auto-generated constructor stub
	}
	...
	public void match(String queryNumber, MatchingQueryTerms queryTerms) {
		...
		for (int i = 0; i < queryLength; i++) {
			...
			LexiconEntry globalEntry = globalLexicon.getLexiconEntry(queryTermStrings[i]);
			...
			wmodel.setDocumentFrequency((double)globalEntry.n_t);
			wmodel.setTermFrequency((double)globalEntry.TF);
			
			logger.debug((i + 1) + ": " + queryTermStrings[i].trim() + " with " + globalEntry.n_t + " documents (TF is " + globalEntry.TF + ").");
