...
import uk.ac.gla.terrier.matching.UnitScorer;

import uk.ac.gla.terrier.matching.models.WeightingModel;
import uk.ac.gla.terrier.querying.termselector.TermSelector;
import uk.ac.gla.terrier.structures.BlockIndexDocument;
...
import uk.ac.gla.terrier.structures.TextWindow;
	...
	*/
	...
	public ExpansionTerm[] expandFromDocuments(
		...
		for (int i=0; i<docIDs.length; i++){
			TextWindow[] windows = BlockIndexDocument.segmentDocument(index, docIDs[i], wSize);
		...
		for (int termid : termidSet.toArray()){
			LexiconEntry entry = lexicon.getLexiconEntry(termid);
			if (entry!=null){
				termidTFMap.put(termid, entry.TF);
				termidNtMap.put(termid, entry.n_t);
			}
		}
		...
		for (String term : terms){
			int termid = lexicon.getLexiconEntry(term).termId;
