 ...
 * size of the underlying file - i.e. the number of entries, or indeed postings, can be variable. 
...
public class BitPostingIndexInputFormat extends FileInputFormat<IntWritable, IntObjectWrapper<IterablePosting>> {
	...
	public InputSplit[] getSplits(JobConf job, int numSplits) throws IOException {
		...
		for(byte i=0;i<fileCount;i++)
		{
			...
			bitPostingStructureFSBlockSizes[i] = fss[i].getBlockSize();
		...
		Iterator<? extends BitIndexPointer> offsetIterator = index.hasIndexStructureInputStream(lookupStructureName+ "-entry")
		...
		long[] blockSize = new long[fileCount];
		//location of the last split for each file
		long[] bitPostingStructureSplitEndOffsets = new long[fileCount];
		
		//how many entries will be in this split, for each file
		int[] entriesInBlock = new int[fileCount];
		//what is the starting id of the next entry split, for each file
		int[] firstEntryOfNextSplit = new int[fileCount];
		
		//number of splits per file, for logging only
		int[] splitsPerFile = new int[fileCount];
		...
		{			
			...
			byte fileId = currentPointer.getFileNumber();
			...
			if (blockSize[fileId] > bitPostingStructureFSBlockSizes[fileId])
			{
				...
				logger.info("File "+ fileId + " split " + splitList.get(splitList.size() -1).toString());
