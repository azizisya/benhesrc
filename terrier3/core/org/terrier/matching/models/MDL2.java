...
public class MDL2 extends WeightingModel
{
	...
	public double score(Posting _p) {
		...
		for(int fi = 0; fi < fieldCount; fi++)
		{
			if (tff[fi] > 0)
				continue;
			final double p = 1.0d / super.numberOfDocuments * fieldsWithTerm * fieldWeights[fi];
			final double tfn_i = this.fieldNormalisations[fi].normalise(tff[fi], fieldLengths[fi], fieldTermFrequencies[fi]);
			tf_q -= tfn_i;
			q -= p;
			double tmp = tfn_i * Math.log(tfn_i / (super.termFrequency*p))/LOG2 + Math.log(tfn_i/super.termFrequency)/(2.0d*LOG2);
			if (tmp > 0)
				score += tmp;
		}
