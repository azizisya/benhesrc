...
/**
 * @author vassilis
 * This matching model was used for the ECIR 2007 paper. It
 * corresponds to the model ML2.
 */
public class BiLGammaFieldMatching extends FieldMatching {

	
	public BiLGammaFieldMatching(Index[] i) {
		super(i);
	}
	...
	public void match(String queryNumber, MatchingQueryTerms queryTerms) {
		...
		System.err.println("Processing query in BiLFieldMatching.");
		for (int i = 0; i < queryLength; i++)
		{
			...
			for (int j=0; j<NumberOfFields; j++) {
				...
				double weight = Double.parseDouble(propertyValue);
				System.err.println("p."+(j+1) + " = " + weight);
				p[j] = p[j] / weight;
				System.err.println("p."+(j+1)+" = " + p[j]);
				String cProperty = "c."+(j+1);
				String wProperty = "weight."+(j+1);
				if (normalisation[j]!=null) normalisation[j].setParameter(Double.parseDouble(ApplicationSetup.getProperty(cProperty, "1.0")));
				if (normalisation[j]!=null) normalisation[j].setFieldWeight(Double.parseDouble(ApplicationSetup.getProperty(wProperty, "1.0")));
			}
			...
			//for each document that matches in at least one field
			{
				...
				double score = -gF.compute_log(TF+1)/log2;
				...
				for(int indexNo=0;indexNo<NumberOfFields;indexNo++)
				{
					...
					if (termFoundInFields[indexNo] && (tmpIndex = Arrays.binarySearch(pointers[indexNo][0],currentDocid)) >= 0) {
						...
						score += (gF.compute_log(tf+1) - tf*Math.log(p[indexNo]))/log2;
						tf_q -= tf;
					}
					q -= p[indexNo];
				}
